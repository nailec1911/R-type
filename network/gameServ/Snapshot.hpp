...
template <typename T>
struct has_vector_conversion<T, std::void_t<decltype(std::declval<T>().operator std::vector<uint8_t>())>>
    : std::true_type {};


template <typename Telement, typename Tother, int nb_others> class Snapshot {
  static_assert(has_equal_operator<Telement>::value,
                "Telement must have operator== overloaded");
  static_assert(has_vector_conversion<Telement>::value,
                "Telement must have cast operator std::vector<u_int8_t> overloaded");
  static_assert(has_equal_operator<Tother>::value,
                "Tother must have operator== overloaded");
  static_assert(has_vector_conversion<Tother>::value,
                "Tother must have cast operator std::vector<u_int8_t> overloaded");
public:
  Snapshot();
  Snapshot(int id, std::array<Tother, nb_others> otherInfo,
           std::map<u_int32_t, Telement> m_eltsMap,
           std::bitset<nb_others> otherUpadte = std::bitset<nb_others>().flip())
      : m_otherUpadte(otherUpadte), m_otherInfo(otherInfo),
        m_eltsMap(m_eltsMap), m_timestamp(time(0)), m_id(id),
        m_acknowledge(false){};

  ~Snapshot();

  time_t getCreationTime() const { return m_timestamp; }
  bool isAcknowledged() const { return m_acknowledge; }
  void setAcknowledge() { m_acknowledge = true; }
  int getId() const { return m_id; }
  std::array<Tother, nb_others> &getOthers() const { return m_otherInfo; }
  std::map<u_int32_t, Telement> &getElements() const { return m_eltsMap; }
