...
rtypeNetwork::rtypeUdpClient::rtypeUdpClient(const std::string &ip, std::uint16_t port)
...
{
    m_socket.async_send_to(asio::buffer(m_sendQueue.front().body, m_sendQueue.front().body.size()), m_endpoint,
...
{
    m_socket.async_send_to(asio::buffer(&m_sendQueue.front().header, sizeof(network::message<rtypeNetwork::rtypeMessageType>)), m_endpoint, 
    [this](std::error_code ec, [[maybe_unused]] std::size_t length) {
        if (!ec)
        {
...
void rtypeNetwork::rtypeUdpClient::sendMessage(const network::message<rtypeNetwork::rtypeMessageType> &msg)
{
