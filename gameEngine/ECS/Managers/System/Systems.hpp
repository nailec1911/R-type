            ...
            for (auto event : events) {
                ...
                if (event.key == EventKey::KeyB) {
                    ...
                    mediator->AddComponent(
                        bullet, Transform{.velX = 4, .velY = 0});
                    mediator->AddComponent(
    ...
    void Update(const std::shared_ptr<Mediator> &mediator)
    {
        ...
        for (size_t i = 0; i < this->mEntities.size(); i += 1) {
            auto &positionEntityOne =
                mediator->GetComponent<Position>(mEntities[i]);
            ...
            for (size_t z = i + 1; z < this->mEntities.size(); z += 1) {
                auto &positionEntityTwo =
                    mediator->GetComponent<Position>(mEntities[z]);
                auto &boxTwo =
                    mediator->GetComponent<BoundingBox>(mEntities[z]);
