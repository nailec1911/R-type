...
ConfigParser::ConfigParser()
{
    std::vector<std::string> allConfigFiles =
        getAllConfigFile("../configEntities");
    ...
    for (const auto& item : allConfigFiles) {
        ...
        for (YAML::const_iterator it = config["Entities"].begin();
             it != config["Entities"].end(); ++it) {
        ...
        for (auto& elem : entitiesType) {
            ...
            newElem.filepath =
                config["Entities"][elem]["sprite"].as<std::string>();
        ...
        m_entitiesType.insert(
            m_entitiesType.end(), entitiesType.begin(), entitiesType.end());
    }
...
inline void ConfigParser::validateFrameNode(const YAML::Node& frameNode)
{
    ...
    if (frameNode["start"].size() != 2 || frameNode["end"].size() != 2) {
        throw ErrorParser(
            "Error: 'start' and 'end' must contain exactly 2 elements.");
    }
}

inline std::vector<rndr::Frame> ConfigParser::parseFrames(
    const YAML::Node& framesNode)
{
    ...
    for (const auto& frameNode : framesNode) {
        ...
        frame.bottomRight = {
            frameNode["end"][0].as<float>(), frameNode["end"][1].as<float>()};
...
void ConfigParser::validateEntity(const YAML::Node& entityNode)
{
    ...
    for (const auto& field : requiredFields) {
        if (!entityNode[field]) {
            throw ErrorParser(
                "Error : Missing field '" + field + "' in configuration file.");
        }
    }
}
...
void ConfigParser::checkfileSprite(const std::string& filepath)
{
    if (!std::filesystem::exists(filepath)) {
        throw ErrorParser(
            "The file which contain the sprite hasn't been found : " +
            filepath);
    }
}
